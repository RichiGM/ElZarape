...
function actualizaTabla() {
    ...
    obj.forEach((elemento, index) => {
        let registro = `
            <tr onclick="selectAlimento(${index});" class="table-row">
                <td>${index + 1}</td>
...
function cambiarEstatus() {
    if (indexAlimentoSeleccionado !== undefined) {
        let combo = obj[indexAlimentoSeleccionado];
        combo.estatus = (combo.estatus === "Activo") ? "Baja" : "Activo";
        actualizaTabla();
        selectAlimento(indexAlimentoSeleccionado);
    }
    limpiar();
}
...
function agregarAlimento() {
    ...
    if (nombre && descripcion && precio && categoria && foto) {
        ...
        alert("Hay campos obligatorios para agregar el Alimento");
    }
}
...
function modificarAlimento() {
    if (indexAlimentoSeleccionado !== undefined) {
        ...
        console.log(obj[indexAlimentoSeleccionado].nombre, document.getElementById("txtNombre").value, nombre);
        actualizaTabla();
        selectAlimento(indexAlimentoSeleccionado);
    }
}
...
function eliminarAlimento() {
    if (indexAlimentoSeleccionado !== undefined) {
        obj = obj.filter((_, index) => index !== indexAlimentoSeleccionado);
        limpiar();
        actualizaTabla();
    }
}
...
function filtrarAlimentos() {
    ...
    filas.forEach(fila => {
        ...
        let descripcion = normalizarTexto(fila.cells[2].textContent);
        let precio = normalizarTexto(fila.cells[3].textContent);
        let categoria = normalizarTexto(fila.cells[4].textContent);
        let estatus = normalizarTexto(fila.cells[6].textContent);

        if (nombre.includes(filtro) || descripcion.includes(filtro) || precio.includes(filtro) || categoria.includes(filtro) || estatus.includes(filtro)) {
            fila.style.display = "";
        } else {
