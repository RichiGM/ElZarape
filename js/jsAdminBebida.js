...
function actualizaTabla() {
    ...
    obj.forEach((elemento, index) => {
        let registro = `
            <tr onclick="selectBebida(${index});" class="table-row">
                <td>${index + 1}</td>
    ...
    filtrarBebidas(); // Aplicar el filtro al cargar la tabla
}
...
function selectBebida(index) {
    ...
    document.getElementById("txtCategoria").value = obj[index].categoria;
...
function cambiarEstatus() {
    if (indexBebidaSeleccionada !== undefined) {
        let combo = obj[indexBebidaSeleccionada];
        combo.estatus = (combo.estatus === "Activo") ? "Baja" : "Activo";
        actualizaTabla();
        selectBebida(indexBebidaSeleccionada);
    }
    limpiar();
}
...
function agregarBebida() {
    ...
    if (nombre && descripcion && precio && categoria && foto) {
        ...
        alert("Hay campos obligatorios para agregar la Bebida");
    }
}
...
function modificarBebida() {
    if (indexBebidaSeleccionada !== undefined) {
        ...
        console.log(obj[indexBebidaSeleccionada].nombre, document.getElementById("txtNombre").value, nombre);
        actualizaTabla();
        selectBebida(indexBebidaSeleccionada);
    }
}
...
function eliminarBebida() {
    if (indexBebidaSeleccionada !== undefined) {
        obj = obj.filter((_, index) => index !== indexBebidaSeleccionada);
        limpiar();
        actualizaTabla();
    }
}
...
function filtrarBebidas() {
    ...
    filas.forEach(fila => {
        ...
        const categoria = normalizarTexto(fila.cells[4].textContent);
        const estatus = normalizarTexto(fila.cells[6].textContent);

        if (nombre.includes(textoBusqueda) || descripcion.includes(textoBusqueda) || precio.includes(textoBusqueda) || categoria.includes(textoBusqueda) || estatus.includes(textoBusqueda)) {
            fila.style.display = "";
        } else {
