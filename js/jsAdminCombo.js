...
function actualizaTabla() {
    ...
    filtrarCombos(); // Aplicar el filtro al cargar la tabla
}
...
function agregarCombo() {
    ...
    if (nombre && descripcion && precio && foto) {
        let newProd = {nombre, descripcion, precio, foto, estatus: "Activo"};
        obj.push(newProd);
...
function filtrarCombos() {
    ...
    filas.forEach(fila => {
        ...
        const estatus = normalizarTexto(fila.cells[4].textContent);
