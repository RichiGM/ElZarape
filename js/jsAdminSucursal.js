...
fetch("http://localhost:8080/ElZarape/json/jsonSucursal.json")
...
// Selecciona un Sucursal y llena el formulario
...
function agregarSucursal() {
    ...
    if (nombre && direccion && latitud && longitud && url && horario && foto ) {
        let newProd = {nombre, direccion, latitud, longitud, url, horario,  foto, estatus: "Activo"};
        obj.push(newProd);
...
// Modifica un Sucursal existente en la lista
...
function filtrarSucursal() {
    ...
    filas.forEach(fila => {
        const nombre = normalizarTexto(fila.cells[2].textContent);
        const direccion = normalizarTexto(fila.cells[3].textContent);
        const latitud = normalizarTexto(fila.cells[4].textContent);
        const longitud = normalizarTexto(fila.cells[5].textContent);
        const url = normalizarTexto(fila.cells[6].textContent);
        const horario = normalizarTexto(fila.cells[7].textContent);
        const estatus = normalizarTexto(fila.cells[8].textContent);
